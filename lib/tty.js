...
Session.prototype.handleCreate = function(cols, rows, func) {
  ...
  // override shell/shellArgs with per-socket config
  var shell = conf.shell;
  var shellArgs = conf.shellArgs;

  var user_config = socket.handshake.config;
  if (user_config) {
    shell = user_config.shell || shell;
    shellArgs = shellArgs.concat(user_config.shellArgs || []);
  }
